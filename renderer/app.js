...
btnPickLocation.addEventListener('click', async () => {
  btnPickLocation.textContent = '⏳ Aguardando clique...';
...
(async () => {
  ...
  cfgX.addEventListener('input', () => { pickXDisplay.textContent = cfgX.value || '—'; });
  cfgY.addEventListener('input', () => { pickYDisplay.textContent = cfgY.value || '—'; });
